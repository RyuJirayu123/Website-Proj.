    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="bg-[#0a1f44] text-white shadow-md">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link to="/" className="w-8 h-8 bg-[#d4a017] rounded flex items-center justify-center font-bold text-white">
              A
            </Link>
            <div>
              <span className="font-bold text-lg">Alpha Capital</span>
              <span className="text-[#d4a017] text-xs font-semibold ml-2 px-2 py-0.5 bg-[#d4a017]/20 rounded">
                Admin Portal
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <Link
              to="/"
              className="text-xs text-white/70 hover:text-white border border-white/20 px-3 py-1.5 rounded transition-colors"
            >
              ไปหน้าเว็บหลัก
            </Link>
            <button
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 flex-1 w-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-bold text-[#0a1f44]">ข้อความติดต่อจากลูกค้า</h1>
          ...
          <button
            ...
            disabled={loading}
            className="inline-flex items-center justify-center gap-2 bg-white border border-gray-200 hover:border-[#d4a017] text-gray-700 px-4 py-2 rounded-lg text-sm transition-colors shadow-sm self-start sm:self-auto"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm flex items-center gap-4">
            ...
            <div>
              <div className="text-2xl font-bold text-[#0a1f44]">{contacts.length}</div>
              <div className="text-gray-500 text-xs">ข้อความทั้งหมด</div>
            </div>
          </div>
        </div>
                  ...
                  {contacts.map((c) => (
                    <tr key={c.id} className="hover:bg-gray-50/60 transition-colors">
                      <td className="py-4 px-6 align-top">
                        <div className="font-semibold text-[#0a1f44]">{c.name}</div>
                        {c.company && (
